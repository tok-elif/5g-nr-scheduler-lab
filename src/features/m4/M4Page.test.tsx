...
describe('M4 page visible states and results', () => {
 ...
 it('shows running with a live status region', () => { const html = status('running'); expect(html).toContain('Simülasyonçalışıyor…'); expect(html).toContain('role="status"'); expect(html).toContain('aria-live="polite"') })
...
expect(html).toContain('URLLC'); expect(html).toContain('mMTC') })
 it('keeps result cards renderable after the worker-owned run has ended', () => expect(results()).toContain('Slicesonuç kartları'))
...
/>)).toContain('Simülasyon tamamlandı')
   expect(renderToStaticMarkup(<M4Results view={createM4ViewModel(snapshot.result!)} />)).toContain('M4 hücreözeti')
