...
npm run dev
```
Windows'ta tek tıkla: **KUR_VE_CALISTIR.bat** bağımlılıkları kurar ve uygulamayı açar.
...
npm run build
```
Güncel durum: **569/569 test geçiyor**, lint 0 uyarı / 0 hata, üretim derlemesi başarılı.
Aynı kontroller her push ve pull request için GitHub Actions üzerinde de çalışır.
## Frekans seçici kanal (opsiyonel)

M0 çekirdeği kanalı UE başına **tek bir wideband SINR** ile temsil eder. Bu varsayım altında
bütün RB'ler birbirinin aynısıdır; bir slotta en iyi metriğe sahip UE bütün RB'lerde en iyidir
ve tahsis kaçınılmaz olarak **bitişik bloklar** hâlinde çıkar. Bu bir kusur değil, wideband
modelin doğru sonucudur.

Gerçek bir hücrede RB'ler arasında SINR farklılaşır (frekans seçici sönümleme): bir UE bandın
bir bölgesinde, başka bir UE başka bir bölgesinde daha iyidir. `config/simulation.json`
içindeki `frequencySelective` bloğu bu davranışı açar:

```jsonc
"frequencySelective": {
  "enabled": false,          // true yapıldığında RB başına scheduling devreye girer
  "stdDevDb": 4,             // RB başına SINR sapmasının standart sapması (dB)
  "coherenceBandwidthRb": 4, // sapmanın düzgün değiştiği RB sayısı
  "seedOffset": 9176         // UE başına sapma üretiminin seed ofseti
}
```

Açıkken:

- Her UE için RB başına SINR profili üretilir. Sapmalar ortalaması sıfır, frekansta
  korelasyonlu ve **seed'e bağlı olarak tekrar üretilebilirdir**.
- Scheduler kararı slot başına değil **RB başına** alınır: her RB o an en yüksek metriğe sahip
  UE'ye gider, backlog'u tükenen UE yarışmadan çıkar. Altı scheduler de (RR, Max C/I, PF,
  M-LWDF, EXP/PF, QDF-PF) değiştirilmeden çalışır.
- Tahsis doğal olarak serpiştirilir; bir UE bitişik olmayan RB'ler alabilir. Kaynak haritasındaki
  RB indeksleri motorun gerçek kararıdır, görselleştirme amaçlı üretilmez.
- Servis kapasitesi UE'ye gerçekten atanan RB'lerin hızları toplanarak hesaplanır.

Kapalıyken (varsayılan) hiçbir kod yolu değişmez; wideband sonuçlar bit düzeyinde korunur.
Round Robin kanal duyarlı olmadığı için açıkken de bitişik blok üretmeye devam eder — bu
